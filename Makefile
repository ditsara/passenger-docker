...
release: tag_latest
	...
	@if ! docker images $(NAME)-ruby23 | awk '{ print $$2 }' | grep -q -F $(VERSION); then echo "$(NAME)-ruby22 version $(VERSION) is not yet built. Please run 'make build'"; false; fi
...
clean_images:
	...
	docker rmi phusion/passenger-ruby22:latest phusion/passenger-ruby23:$(VERSION) || true
